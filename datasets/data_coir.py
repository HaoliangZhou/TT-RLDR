...
class CIRR(Dataset):
    ...
    vis_mode=False, test=False, root='/data/path/CIR_task', if_clip=True):
        self.mode = mode
        ...
        if self.test:
            ...
            if self.mode == 'caps':
                self.json = os.path.join(self.root , 'cirr/captions/cap.rc2.test1.json')
            else:
                self.json = os.path.join(self.root, 'cirr/image_splits/split.rc2.test1.json')
        else:
            if self.mode == 'caps':
                self.json = os.path.join(self.root, 'cirr/captions/cap.rc2.val.json')
            else:
                self.json = os.path.join(self.root, 'cirr/image_splits/split.rc2.val.json')
        logging.debug(f'Loading json data from {self.json}.')
        data = json.load(open(self.json, "r"))                                
        self.ref_imgs = []
        self.target_imgs = []
        self.target_caps = []
        # self.target_caps_replace = []
        if self.test:
            self.init_test(data)
        elif self.mode == 'caps':            
            self.init_val(data)
    ...
    def return_testdata(self, idx):
        if self.mode == 'caps':
                ...
                tar_img_path = os.path.join(self.root_img, tar_path)
                target_images = self.transforms(Image.open(tar_img_path))

                target_cap = self.target_caps[idx]
                text_with_blank_raw = 'a photo of * , {}'.format(target_cap)
                caption_only = target_cap
                ref_text_tokens = text_with_blank
                return ref_images, target_images, ref_text_tokens, caption_only, str(self.ref_imgs[idx]), self.pairids[idx], text_with_blank_raw 
        else:
            tar_path = str(self.target_imgs[idx])
    ...
    def return_valdata(self, idx):
        if self.mode == 'caps' and not self.vis_mode:
            ...
            target_cap = self.target_caps[idx]
            # target_cap_replace = self.target_caps_replace[idx]
            text_with_blank = 'a photo of * , {}'.format(target_cap)
            caption_only = target_cap
            ref_text_tokens = text_with_blank
            return ref_images, target_images, ref_text_tokens, caption_only, str(self.ref_imgs[idx]), str(self.target_imgs[idx]), target_cap
        else:
            tar_path = str(self.target_imgs[idx])
...
class FashionIQ(Dataset):
    def __init__(self, cloth, transforms, is_train=False, vis_mode=False, \
        mode='caps', is_return_target_path=False, root='./data/path/CIR_task/', if_clip=True):
    ...
    def return_all(self, idx):
        ...
        # text_with_blank = 'a photo of <|replace|> , {} and {}'.format(cap2, cap1)
        token_texts = text_with_blank
        if self.is_return_target_path:
            return ref_images, target_images, token_texts, token_texts, str(self.target_imgs[idx]), str(self.ref_imgs[idx]), cap1
        else:
...
class CsvCOCO(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masked_path = os.path.join(self.root_img.replace('val2017', 'val2017_masked'), str(self.images[idx]))
        ...
        text_with_queryclass = 'a photo of * and {} and {}'.format(query_class, " and ".join(other_classes))
        # text_with_queryclass = 'a photo of <|replace|> and {} and {}'.format(query_class, " and ".join(other_classes))
        text_with_queryclass_origin = text_with_queryclass
        raw_text = text_with_queryclass
        text_full = 'a photo of {} and {}'.format(query_class, " and ".join(other_classes))        
        return image, region_image, text_full, text_with_blank, text_with_queryclass, str(self.images[idx]), raw_text, text_with_queryclass_origin
...
class CsvDataset_replace(Dataset):
    def __init__(self, input_filename, transforms, img_key, caption_key, replaced_caption_key, sep="\t",
                 return_data_identifier=False, return_filename=False):
        logging.debug(f'Loading csv data from {input_filename}.')
        df = pd.read_csv(input_filename, sep=sep)
        self.images = df[img_key].tolist()
        self.captions = df[caption_key].tolist()
        self.replaced_captions = df[replaced_caption_key].tolist() # used for training
        self.transforms = transforms
        self.return_data_identifier = return_data_identifier
        logging.debug('Done loading data of {} samples'.format(len(self.images)))
        self.return_filename = return_filename
    ...
    def __getitem__(self, idx):
        ...
        if self.return_filename:
            return images, str(self.images[idx])
        texts = str(self.captions[idx])
        replaced_texts = str(self.replaced_captions[idx])

        if self.return_data_identifier:
            return images, texts, 0
        return images, texts, replaced_texts
...
def get_csv_dataset(args, preprocess_fn, is_train, input_filename=None):
    ...
    dataset = CsvDataset_replace(
        input_filename,
        preprocess_fn,
        img_key=args.csv_img_key,
        caption_key=args.csv_caption_key,
        replaced_caption_key=args.csv_replace_key,

...
class TTAMethod(nn.Module):
    def __init__(self, cfg, model, combiner, num_classes):
        ...
        print("Configure model and optimizer...")
        self.configure_model()

        if cfg.MODEL.ADAPTATION in ["tent", "cotta", "sar", "roid", "cmf", "deyo","kd","reward"]:
            self.fixed_v_lay_list, self.fixed_t_lay_list = self.fixed_layer_list(v_lay=cfg.CLIP.FIXED_VISUAL_LAYER, t_lay=cfg.CLIP.FIXED_TEXTUAL_LAYER) 
            print("Fixed visual layers, len({}): {}, fixed textual layers, len({}): {}".format(len(self.fixed_v_lay_list), self.fixed_v_lay_list, len(self.fixed_t_lay_list), self.fixed_t_lay_list))
            self.params, self.param_names = self.collect_params(self.fixed_v_lay_list, self.fixed_t_lay_list)
        else:
    ...
    def forward(self, x=None, y=None, z=None):
        ...
        if x is not None and y is None:
            ...
            for _ in range(self.steps):
                outputs = self.forward_and_adapt(x, y, z)
            return outputs
    ...
    def setup_optimizer(self):
        for nm, p in self.model.named_parameters():
            if nm not in self.param_names:
                p.requires_grad = False
        enable_param = []
        for nm, p in self.model.named_parameters():
            if p.requires_grad:
                enable_param.append(nm)
        print("Enable_param len({}): {}".format(len(enable_param), enable_param))
    ...
    def fixed_layer_list(self, v_lay=None, t_lay=None):
        freeze_layers_visual = []
        freeze_layers_textual = []

        # Freeze visual layers up to v
        if v_lay is not None:
            freeze_layers_visual.extend([f"model.visual.transformer.resblocks.{i}." for i in range(v_lay)])

        # Freeze text layers up to t
        if t_lay is not None:
            freeze_layers_textual.extend([f"model.transformer.resblocks.{i}." for i in range(t_lay)])

        return freeze_layers_visual, freeze_layers_textual

    def get_number_trainable_params(self):
        trainable = sum(p.numel() for p in self.params) if len(self.params) > 0 else 0
        total = sum(p.numel() for p in self.model.parameters())
        logger.info(f"#Trainable params len({len(self.param_names)}), Trainable/total parameters: {trainable:,}/{total:,} \t Ratio: {trainable / total * 100:.3f}% ")
        # logger.info(f"#Trainable params, len({len(self.param_names)}), : {self.param_names}")
...
class TTAMethod_with_reward(nn.Module):
    ...
    def fixed_layer_list(self, v_lay=None, t_lay=None):
        ...
        # Freeze visual layers up to v
        if v_lay is not None:
            freeze_layers_visual.extend([f"clip_model.visual.transformer.resblocks.{i}." for i in range(v_lay)])


        # Freeze text layers up to t
        if t_lay is not None:
            freeze_layers_textual.extend([f"clip_model.transformer.resblocks.{i}." for i in range(t_lay)])
        return freeze_layers_visual, freeze_layers_textual
